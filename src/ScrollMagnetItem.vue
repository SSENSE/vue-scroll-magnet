  ...
  export default {
    ...
    methods: {
      ...
      setMagnetStatus(nearestContainer) {
        ...
        // The distance scrolled within the bounds (Number)
        this.scrollDist = (nearestContainer.scrollTop + this.height)

        // The maximum scrollable distance (Number)
        this.scrollEnd = (nearestContainer.offsetTop + (nearestContainer.height - this.offsetTopPad))

        // Is the item still within the bounds? (Boolean)
        this.isWithinHeight = (this.scrollDist < this.scrollEnd)

        // Is the item currently scrolling? (Boolean)
        this.isScrolling = (
          (nearestContainer.scrollTop + this.offsetTopPad)
          >= nearestContainer.offsetTop && this.isWithinHeight
        )

        // Should the item stick to the bottom of the bounds? (Boolean)
        this.isBottomed = (this.scrollDist >= this.scrollEnd)

        // Item"s CSS top value (Number)
